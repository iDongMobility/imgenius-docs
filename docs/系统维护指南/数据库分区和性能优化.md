...
***注意：本文中新建的文件组、文件以及分区的数量都只是举例，实际需要建多少文件、文件组，要先根据实际数据库的使用情况规划好分多少个区。虽然本文中步骤上创建文件组和文件在创建分区之前，但实际使用时，一定是先规划好分多少个区再开始的。同时要有一个通用分区，在完成数据库的分区后，所有不属于规划分区内的数据，将会自动归入通用分区。所以在创建分区方案时，是要比这个新建的数量多出一个的。***
...
***注意，语句中的ExecuteTimeDay是H_Ext_KPI这张表里的列，在为其他表分区时，要改成对应表里的列***
