...
* 如果在运行服务较多，对于系统资源的占用比较大，建议勾选上保持索引联机，然后在空闲时间再运行此计划。
...
* PostgreSQL自10.X版本起引入了内置分区表的功能，简化了创建分区表的操作，最终简单三步就能够创建分区表。内置分区支持范围分区（RANGE）、列表分区（LIST），以及11.X版本后添加的哈希分区（HASH）。
...
* 在进行任何重建索引的操作之前，首先确保对数据库进行了备份，以防止意外情况发生。PostgreSQL数据库的备份请查看[数据备份和还原](/系统维护指南/数据备份和还原.md)
