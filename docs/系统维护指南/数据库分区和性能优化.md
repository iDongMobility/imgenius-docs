...
* 由于系统中，表名都是固定的，而PG暂不支持在原有表上进行分区，所以要创建新的表为分区表，并且名称要为h_ext_kpi，这里就需要把原有表的名称改掉。在PGAdmin4里选中h_ext_kpi表，右键-属性，然后修改表名为h_ext_kpi_old。
...
* 这里语句的意思是，创建一个分区表名为h_ext_kpi，其中有一列是executetimeday，类型是int。并按照范围分区，将executetimeday设置为分区键。
...
* 在分区之前，一定要对表中的数据有清晰的认知，然后做好规划。这里是将20220101-20220801范围内的数据放进一张分区表中，作为一张历史数据表。之后每月的数据放进一张分区表中。在实际使用中，也可以随时新增分区表来存放新的范围数据。
...
* 由于PG没有全局索引的概念，所以主键或者唯一键必须要包含分区键，否则会报错。
