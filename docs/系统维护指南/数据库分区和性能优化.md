...
虽然本文中步骤上创建文件组和文件在创建分区之前，但实际使用时，一定是先规划好分多少个区再开始的。
并且一定是对需要进行分区的数据库表的数据有清晰的认识才可以进行规划，简单来说，就是知晓数据库中的数据内容，然后根据内容的特性（比如知道数据库是从哪年开始用的，用了多少年，就可以在数据量很大的表中，根据某一列值为年或月的作为标识去进行规划和实际的操作分区）来规划。
同时需要有一个通用分区，在完成数据库的分区后，所有不属于规划分区内的数据，将会自动归入通用分区。所以在创建分区方案时，是要比这个新建的数量多出一个的。
:::tip 注意
...
to filegroup ExtKPI20150701
alter database imgenius add file 
(name=N'KPI20151001',filename=N'D:\WorkSoftWare\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\DATA\KPI7.ndf',size=5Mb,filegrowth=5mb)
to filegroup ExtKPI20151001
alter database imgenius add file 
...
:::tip 注意
例子中的边界值要根据实际数据库中的值来进行修改，不可以直接复制语句执行。例子中的表为H_Ext_KPI，如其中的第一个值'20140401'，需要修改为实际数据库里的值。
:::tip 注意
...
GO
BEGIN TRANSACTION
CREATE PARTITION FUNCTION [kpi](int) AS RANGE LEFT FOR VALUES (N'20140401', N'20140701', N'20141001', N'20150101', N'20150401', N'20150701', N'20151001', N'20160101')
```
...
```
CREATE PARTITION SCHEME [kpi] AS PARTITION [kpi] TO ([ExtKPI20140401], [ExtKPI20140701], [ExtKPI20141001], [ExtKPI20150101], [ExtKPI20150401], [ExtKPI20150701], [ExtKPI20151001], [ExtKPI20160101], [ExtKPI20160401])
```
...
ALTER TABLE [dbo].[H_Ext_KPI] DROP CONSTRAINT [PK_H_Ext_KPI]:
这句代码的作用是从名为 [H_Ext_KPI] 的表中删除名为 [PK_H_Ext_KPI] 的主键约束。

ALTER TABLE [dbo].[H_Ext_KPI] ADD CONSTRAINT [PK_H_Ext_KPI] PRIMARY KEY NONCLUSTERED ([ID] ASC) WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 85) ON [PRIMARY]:
这句代码的作用是向表 [H_Ext_KPI] 添加一个名为 [PK_H_Ext_KPI] 的非聚集索引主键约束，其中 [ID] 列被指定为主键。其他的参数设置指定了约束的行为和存储位置。

CREATE CLUSTERED INDEX [ClusteredIndex_on_kpi_636451424306135769] ON [dbo].[H_Ext_KPI] ([ExecuteTimeDay]) WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [kpi]([ExecuteTimeDay]):
这句代码的作用是在表 [H_Ext_KPI] 上创建一个名为 [ClusteredIndex_on_kpi_636451424306135769] 的聚集索引。索引的键是 [ExecuteTimeDay] 列。其他的参数设置定义了索引的行为和存储位置。

DROP INDEX [ClusteredIndex_on_kpi_636451424306135769] ON [dbo].[H_Ext_KPI]:
这句代码的作用是从表 [H_Ext_KPI] 中删除名为 [ClusteredIndex_on_kpi_636451424306135769] 的索引。

COMMIT TRANSACTION:
这句代码的作用是提交当前的事务。它将保存上述修改的内容。
```
