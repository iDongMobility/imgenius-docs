...
* 在SDC中，打开资产类型和资产的属性，可以看到新增了外部属性选项，勾选上后，服务运行的情况下，该资产会在Thingsboard的设备中自动创建出来，可以在thingsboard中为该设备配置出遥测属性和动态数据，属性名称请与SDC中该资产属性名称一致，如何配置请查看
<a href={useBaseUrl('docattachment/TB配置仿真及报警.docx')} target="_blank">TB配置仿真及设置报警</a>
...
* 从Thingsboard中拿到的值会实时更新到资产属性中
  ...
  * 在EOC上的资产总览-属性中，刷新可看的数据在变化，可以在历史趋势和表格里看所有记录
...
* 实际场景中设备的属性数据会通过http、mqtt、CoAP等设备连接协议进行传送
...
### EOC作业项Word和Excel视图支持使用第三方预览服务

* EOC端作业项的Word和Excel视图功能优化，支持第三方预览服务
...
* 在SMC中添加了相应的系统设置
  * 在系统参数中添加了使用外部文档服务选项，勾选上，地址栏中填写'第三方服务地址=$DocUrl$'，$DocUrl$为文件存放在第三方服务地址，比如：
...
* 配置完成后，当用户在EOC端和终端对新建字段进行编辑后，填写的值会自动更新至存储区域表和物料表中
