...
* 语音识别功能主要目的是在工厂巡检环境下，通过识别巡检人员语音以简化输入过程。现阶段，只实现数字和字符串类型输入项的语音转文本输入。手机端移动工作站应用采集语音，把语音转化为二进制数据流，通过WebSocket的连接方式发送给后台语音识别服务，后台服务把识别后的文本回传给移动工作站应用，移动工作站应用把识别文本自动录入到当前执行项的输入框中。
...
* 组件中包含了所有已完成开发可供使用的小组件，图层里列出了所有已在工作空间里使用中的组件，而大部分编辑都在工作空间和页面设置中完成，其中页面设置为整个页面的设置，另外左上角的按钮可以隐藏/显示几大板块。
...
* 编辑过程中也可以随时预览。在编辑完成后，注意要先点击保存再回到主页。
...
* 左侧为菜单栏，分别为组织架构、资产管理、列表管理、业务流程、系统配置、作业管理、系统。
...
* 如果要使用OPCDA数据源，是需要手动注册opcdaauto.dll的。现在系统会判断是否存在，若没有将会自动进行注册。
