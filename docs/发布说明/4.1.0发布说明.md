...
* 以DA为例，在建完数据源之后，新建一个资产类别为OPCDAClient的资产，作为数据源，点开资产属性。假设使用了Matrikon OPC Server Simulation建了一个点位，资产属性这里Server的值就来自其client中，Run的值代表运行或停止，填1或0即可，Group是数组模式，其中Name名称自定义，UpdateRate为刷新时间，Deadband写0即可，这里是为了满足当点位很多并且每个点位数据变化的时间要求不一样时，能够很好的进行区分接收所做的配置。
