...
* 格式前后固定，只需要修改两个list中的各项名称就行。根据本地解决方案中的作业组模板名和分组名修改其中的值，其中：
  ...
  * TaskGroupTemplateNames:作业组模板名称，选填（2022/11/29之后的更新包版本为TaskGroupTemplateIDs，是作业组模板ID，不是作业组模板名）
  * GroupNames:作业组组名，选填
...
* 同时，打印页面也支持自定义显示的内容和样式，需要配置脚本和终端自定义页面
...
* 支持添加终端物料查询扩展页面，在iis中添加一个应用程序，再在EOC端新增一个终端自定义查询，然后在终端进入此页面。

* 首先在iis的EOC目录中添加应用程序，点此下载：<a href={useBaseUrl('docattachment/dayin.zip')} target="_blank">打印应用程序</a>，下载后解压，再如图添加即可
...
* 添加完成后，终端同步显示此页面，进入选择物料打印即可，此页面除了查询物料信息外，也可用于打印物料信息。
...
* 重构了EOC上选择图标的对话框，对图标进行了详细的分类，更改了排列方式，并增加了更多的图标。
...
* 在SMC中进行部署时，也将外部数据服务和关联关系服务加入，点击启用服务时勾选上，可以与原有的五大服务一起统一部署和运行

  <img alt=" " src={useBaseUrl('docimg/bushu4106.png')} />
