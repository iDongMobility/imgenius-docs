...
* 部署包：包含了部署工具文件夹，EOC部署包，Api部署包，Web配置中心部署包和终端扩展部署包。
...
* apk：包含imMW.apk，不含AIC测温测振、人脸识别、远程指导、钛准定位和打印插件的应用；imMW_Face.apk，含人脸识别和打印插件的应用；imFigure.apk，大屏应用；imTagTool.apk，标签读写应用。
