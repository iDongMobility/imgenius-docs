...
* 配置：
  ...
  * 更多
    ...
    * 下图为使用工具AD.Net.exe可以查看域服务器有多少用户，并可查看用户的属性字段。（此工具位于tools文件夹中）
...
:::tip 注意
必须安装与安装程序一同打包的压缩包中，名为dotnet文件夹里的dotnet程序。
:::tip 注意
...
// 定义消息主体
var body = {
    'Modes': [ // 订阅方式，1代表Web端，2代表终端，3代表邮件，4代表短信
        1,
        3
    ],    
...
* 对于第一个函数调用，参数的含义如下：
  ...
  * '': 消息类型，为空表示订阅所有消息。// 如TaskGroupPendingAudit，为待审核消息；TaskGroupPendingExecute，为待执行消息。与EOC端消息类型管理里的消息类型名一致。
  * '1,2': 订阅方式，表示订阅方式的标识。1代表Web端，2代表终端，3代表邮件，4代表短信
...
var userNameList = property.Value;
imLogWriter.WriteInfo('人员名称:' + userNameList); // 输出当前用户名称
var errorMsg = ScriptEngine.SaveDynamicSubscribe(taskGroupID, userNameList, 'MT_TaskGroupAuditBeforeDeadLine,MT_TaskGroupAuditAfterDeadLine', '1,2', '审核', bpID, '审核1');
if (errorMsg) {
    imLogWriter.WriteError(errorMsg); // 若保存动态订阅项出错，则输出错误信息
}
```
...
* 基础版应用和带人脸识别、打印等插件版应用都位于EOC部署路径下imMW文件夹中，如C:\inetpub\wwwroot\imMW；imFigure应用位于EOC部署路径下imFigure文件夹中的system文件夹里，如C:\inetpub\wwwroot\imFigure\system。
...
* 首先，通过在SDC中添加内置属性`$UseExceptionColor`，其值固定为`BackgroundColor`或`FontColor`,分别为背景颜色和字体颜色，作业项与自身关联的例外等级颜色保持一致。如果在当前作业组中，有关联了例外等级的作业（项），在特殊视图Word和Excel中，会根据属性的值变化该作业（项）的背景颜色或字体颜色。
