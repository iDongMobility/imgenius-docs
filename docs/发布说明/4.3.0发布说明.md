...
* 在Web配置和管理中心-作业管理-作业组模板中，新增了自定义表单功能，此功能并不是表单模式的增强，而是可以自定义作业组属性在EOC和终端上的显示样式，并且可以较为简单的去配置出在SDC里无法配置的或配置起来很复杂的需求。注意，配置自定义表单的作业组模板，作业和作业项不会显示，也不能执行。如果想要在作业树中查看的样式为自定义表单样式，则需要选择上表单模式。
...
* 注意：自定义表单中的配置在保存后，要在SDC中重新打开解决方案，重新发布该作业组模板，才会生效。
...
* 然后，为这个实体类型新建属性，新建完属性后会在数据表中自动增加相应的字段，支持所有数据类型。
...
// 调用函数获取数据，参数分别为实体类型名称、要获取的字段名（与实体属性中自定义的名称一致）、查询条件（比如写为'故障代码', 'ID,Itemnumber', 'Itemnumber=LNE-O-S17'，则返回的就是值为LNE-O-S17的Itemnumber列表）
ScriptEngine.GetCustomEntityData('故障代码', 'ID,Itemnumber', '', function (_list) {
    var list = [];
    if (_list && _list.length > 0) {
        for (var i = 0; i < _list.length; i++) {
            var row = _list[i];
            // 创建一个对象obj，该对象具有两个属性：ID和Name，属性值分别为row对象中键为ID和Itemnumber的值
...
* 之后可以在该作业项的逻辑脚本条件中，填加下面的脚本，上面说到终端上执行时，该作业项可选择的值就是故障代码中ItemNumber字段的值，而添加上此脚本，该作业项选完值后，会将ItemNumber值所对应的Description值自动填入"故障原因"作业项中，实现类似于自定义列表关联关系的功能。
...
if (taskItemValue) {
    ScriptEngine.GetCustomEntityData('故障代码', 'ID,Itemnumber,BI_Description', '', function (_result) {
        ...
        if (reasonTask) {
            var value = filter.length > 0 ? filter[0]['BI_Description'] : ''; //BI_Description为描述
