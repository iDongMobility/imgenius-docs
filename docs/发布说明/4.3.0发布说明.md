...
* 在Web配置与管理中心里，增加了自定义接口功能，可以自己定义一个接口，在定义的接口中写好脚本。在系统需要此脚本时，可以直接调用这个接口来实现执行该脚本的功能。如果在一个解决方案中有比较多的内容和用途重复的脚本，使用此功能可以更快速简单的进行配置，并且对于脚本的管理也更清晰；而如果在解决方案中用到的脚本都是不同的，使用此功能或在配置中直接写脚本，是没有本质区别的。
