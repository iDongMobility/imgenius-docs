...
var resultName = ScriptEngine.GetResultName();
console.log(11);
var orgUnitNames = ['a', 'b']; //组织单元名
var postIDs = ['69887a1f-9e15-4e0b-8197-dc81e38b3608', '4e7dabd6-8d6a-4cd6-a9b9-d7cd1d1f5eb3']; //维修、班长

var orgUnitsObj = {};
function getOrgUnitIDs(_orgUnitName, _callback) {
    ScriptEngine.GetOrgUnits(_orgUnitName, true, false, function (_list) {
        var orgUnitIDs = [];
        orgUnitsObj[_orgUnitName] = [];
        if (_list && _list.length > 0) {
            orgUnitIDs.push(_list[0].ID);
            for(var i=0;i<_list.length;i++){
                orgUnitsObj[_orgUnitName].push(_list[i].ID);
            }
        }
        _callback(orgUnitIDs);
    })
}

function getUsersByOrgUnitName(_orgUnitName, _usersByOnePostID, _callback) {
    getOrgUnitIDs(_orgUnitName, function (_orgUnitIDs) {
        if (_orgUnitIDs && _orgUnitIDs.length > 0) {
            var users = [];
            function getOne(index) {
                if (index >= _orgUnitIDs.length) {
                    _callback(users);
                } else {
                    ScriptEngine.GetAjaxToApiServer('UserConfig', 'GetByOrgUnitID', 'orgUnitID=' + _orgUnitIDs[index], '', 'get', function (_usersByOrgUnitID) {
                        if (_usersByOrgUnitID && _usersByOrgUnitID.length > 0) {
                            for (var i = 0; i < _usersByOrgUnitID.length; i++) {
                                var user = _usersByOrgUnitID[i];
                                var find = users.find(function (_item) { return _item.ID == user.ID });
                                var findByPost = _usersByOnePostID.find(function (_item) { return _item.ID == user.ID });
                                if (findByPost && !find) {
                                    users.push(user);
                                }
                            };
...
var judgeByRoles = function (_users, _postID, _orgUnitName, _callback) {
    var users = [];
    if (_users && _users.length > 0) {
        var judgeOne = function (_index) {
            if (_index >= _users.length) {
                _callback(users);
                return;
            }
            ScriptEngine.GetAjaxToApiServer('UserConfig', 'GetRoles', 'userID=' + _users[_index].ID, '', 'get', function (_roles) {
                if (_roles && _roles.length > 0) {
                    var find = _roles.find(function (_item) { return _item.RelatedPost && _item.RelatedPost.ID == _postID && _item.RelatedOrgUnit && orgUnitsObj[_orgUnitName] &&  orgUnitsObj[_orgUnitName].indexOf(_item.RelatedOrgUnit.ID)>-1});
                    if (find) {
                        users.push(_users[_index]);
                    }
                }
                _index++;
                judgeOne(_index);
...
var users = [];
var getOneCombinationUsers = function (_index) {
    if (_index >= postIDs.length > 0) {
        ScriptEngine[resultName] = users;
        return;
    }
    var postID = postIDs[_index];
    var orgUnitName = orgUnitNames[_index];
    //获取该岗位的所有人员
    ScriptEngine.GetAjaxToApiServer('UserConfig', 'GetByPostID', 'postID=' + postID, '', 'get', function (_usersByPostID) {
        //获取属于该组织单元的所有人员
        getUsersByOrgUnitName(orgUnitName, _usersByPostID, function (_usersByOrgUnitName) {
            judgeByRoles(_usersByOrgUnitName, postID, orgUnitName, function (_filterUsers) {
                if (_filterUsers && _filterUsers.length > 0) {
                    for (var i = 0; i < _filterUsers.length; i++) {
                        var user = _filterUsers[i];
                        var find = users.find(function (_item) { return _item.ID == user.ID });
                        if (!find) {
                            users.push(user);
                        }
                ...
                _index++;
                getOneCombinationUsers(_index);
            })
        })
    })
}

getOneCombinationUsers(0);
